...
{
    public class ClaimExpenseChargeCodeService : BaseService
    {
        public ClaimExpenseChargeCodeService(ClaimContext context, IHttpContextAccessor httpContextAccessor, ILogger<ClaimExpenseChargeCodeService> log)
           : base(context, httpContextAccessor, log)
        {
        }
        public async Task<IEnumerable<ClaimExpenseChargeCodeResponseDTO>> Get(Expression<Func<ClaimExpenseChargeCode, bool>>? predicate = null)
        {
            try
            {
                if (predicate is null)
                    predicate = x => x.Id.ToString() != "";
                return await context.ClaimExpenseChargeCode.Where(predicate)
                ...
                .Select(select => new
                {
                    ...
                    ClaimExpenseId = select.Tat.Tat.Tat.ClaimExpenseId,
                ...
                }).OrderBy(p => p.ClaimExpenseId).OrderBy(p => p.SeqNo).Project().To<ClaimExpenseChargeCodeResponseDTO>().ToListAsync();
            }
            catch (Exception ex)
            {
        ...
        public async Task AddOrUpdateOrDelete(ClaimExpenseChargeCodeRequestDTO data)
        {
            try
            {
        ...
        public async Task<Guid> Add(ClaimExpenseChargeCodeRequestDTO data)
        {
            try
            {
                var model = new ClaimExpenseChargeCode();
                model.MapFrom(data);
                ...
                model.IsActive = true;
                await context.ClaimExpenseChargeCode.AddAsync(model);
        ...
        public async Task<ClaimExpenseChargeCodeResponseDTO> Update(ClaimExpenseChargeCodeRequestDTO data)
        {
            try
            {
                var model = await context.ClaimExpenseChargeCode.AsNoTracking().FirstOrDefaultAsync(x => x.Id == data.Id);
                if (model == null)
                    throw new Exception("Claim expense charge code not found.");

                //model.MapFrom(data);
                var newModel = new ClaimExpenseChargeCode();
                newModel.MapFrom(model);

                //model.MapFrom(data);
                newModel.ClaimId = data.ClaimId;
                newModel.ClaimExpenseId = data.ClaimExpenseId;
        ...
        public async Task<ClaimExpenseChargeCodeResponseDTO> UpdateIsActive(Guid id)
        {
            try
            {
                var model = await context.ClaimExpenseChargeCode.FirstOrDefaultAsync(x => x.Id == id);
                if (model == null)
                    throw new Exception("Claim expense charge code not found.");
        ...
        public async Task Delete(Guid id)
        {
            try
            {
                ...
                context.ClaimExpenseChargeCode.Remove(model);
                await context.SaveChangesAsync();
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<ClaimExpenseChargeCodeResponseDTO>> GetById(Guid id, Expression<Func<ClaimExpenseChargeCode, bool>>? predicate = null)
        {
            try
            {
                if (predicate is null)
                    predicate = x => x.Id.ToString() != "" && x.Id == id;
                return await context.ClaimExpenseChargeCode.Where(predicate).Project().To<ClaimExpenseChargeCodeResponseDTO>().ToListAsync();
            }
            catch (Exception ex)
            {
                ErrorServiceHandler(ex);
        ...
        public async Task<List<ClaimExpenseChargeCodeResponseDTO>> GetByClaimId(Guid id, Expression<Func<ClaimExpenseChargeCode, bool>>? predicate = null)
        {
            try
            {
                if (predicate is null)
        ...
        public async Task<bool> CheckExpenseIsActive(ClaimExpenseChargeCodeRequestDTO data)
        {
            try
            {
                bool isactive = false;
                var model = await context.ClaimExpense.FirstOrDefaultAsync(x => x.Id == data.ClaimExpenseId);
                if (model == null)
                    throw new Exception("Claim expense charge code not found.");
                else
